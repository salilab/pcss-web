...
sub get_index_page {
	...
	$q->h2({-align=>"center"},
	       "PCSS: Peptide Classification using Sequence and Structure <br> NOTE: 3/23/11 This site is currently under construction and may not return expected results") . 
...
sub process_user_input {
	...
	if ($applicationSpecification eq "D"){
	    ...
	    my $stats = $self->processUserSpecifiedPeptides($applicationFh, $errorHandling, $directory, $peptideLength);
...
# PARAM  $peptideLength: length of the peptides as defined by the model (application mode); 0 (training mode; set here)
# RETURN $stats: statistics about which peptides were found or not.
###########################################################################################################################################################
sub processUserSpecifiedPeptides{
    my ($self, $peptideFh, $errorHandling, $directory, $peptideLength) = @_;

    my ($peptideFileInfo) = $self->readAndValidatePeptideFile($peptideFh, $directory, $peptideLength);
    my $stats;
...
sub readAndValidatePeptideFile{
    my ($self, $peptideFh, $directory, $peptideLength) = @_;
	...
	if ($peptideLength == 0){
	    $peptideLengthReferenceLine = $line;
	    $peptideLength = length($peptideSequence);
	}
	else {
	    if (length($peptideSequence) != $peptideLength){  #check all peptides are the same length
		if ($self->getParam("server_mode") eq "training"){
		    throw saliweb::frontend::InputValidationError("Content error in peptide file: All peptides in the set must be the same length.  Peptide sequences of different lengths were found in the following two lines:\n$peptideLengthReferenceLine\n$line");
		}
		else {
		    throw saliweb::frontend::InputValidationError("Content error in peptide file: All peptides must be the same length as the peptides that were used to train the model. The model you are trying to use was trained on peptides of length $peptideLength. A peptide sequence with a different length was found in the following line:\n$line");
		}
	    }
	}
...
#
# PARAM  $sequenceInfo: map of uniprot accessions to modbase sequence IDs of the form $sequenceInfo->{$uniprotAccession}->{modbaseSeqId} = $modbaseSeqId
...
sub validatePeptideSequenceInfo{
    ...
    foreach my $uniprotAccession (keys %$sequenceInfo){
	$stats->{input}->{accessions}++;

	#get accession info
	my $sequence = $sequenceInfo->{$uniprotAccession}->{sequence};
	my $modbaseSeqId = $sequenceInfo->{$uniprotAccession}->{modbaseSeqId};
...
#        InternalError if certain sanity checks fail.
# RETURN $sequenceInfo, a hash containing all the results of the form $sequenceInfo->{$uniprotAccession}->{modbaseSeqId} = $modbaseSeqId
#                                                                                                       ->{sequence}     = $proteinSequence
###########################################################################################################################################################################
sub getSequencesFromModbase{
	...
	if ($modbaseSeqId){   #found sequence in modbase, add it to $sequenceInfo
	    ...
	    $sequenceInfo->{$uniprotAccession}->{modbaseSeqId} = $modbaseSeqId; 
	    $sequenceInfo->{$uniprotAccession}->{sequence} = $sequence;
	    $foundSequencesCount++;
	}
	else {
...
sub processUserCreatedModel{
    ...
    # possible errors -- no separator, fewer than three benchmark lines, fewer than three svm lines? also check the svm model file for number of training examples and other input lines
